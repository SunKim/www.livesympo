...
<head>
	...
	form.enter-form td.td-input {
		width: 70%;
	}

	form.enter-form td.td-input span {
		width: 5rem;
	}

	form.enter-form td.td-input input {
		width: calc(100% - 7rem);
		color: #333;
	}

	form.enter-form td.td-btn {
		width: 30%;
	}
	...
	table.table-apply th.required::after {
		content: '*';
		display: inline-block;
		margin-left: 3px;
		font-size: 15px;
	...
	@media (max-width: 768px) {
		...
		div.form-container form.enter-form {
			margin: 0 auto;
		}
	...
	@media (min-width: 1200px) {
		...
		div.agenda-container {
			display: inline-block;
			width: 50%;
			height: <?=$project['ENT_THME_HEIGHT'] ?>px;
		}

		div.agenda-container img.agenda-img {
			height: <?=$project['ENT_THME_HEIGHT'] ?>px;
		}
...
?>
			<div class="apply-btn-container" style="text-align: <?= $project['APPL_BTN_ALIGN'] ?> !important;">
				<button class="btn-apply <?= $project['APPL_BTN_ROUND_YN'] == 1 ? 'round'  : '' ?>"
					style="background: <?= $project['APPL_BTN_BG_COLR'] ?>; color: <?= $project['APPL_BTN_FONT_COLR'] ?>;"
					onclick="openApply();">사전등록</button>
			</div>
			<?php
    }
?>
		...
		<section class="form">
			...
			<div class="form-container"
				style="background: <?= $project['ENT_THME_COLR'] ?>; color: <?= $project['ENT_THME_FONT_COLR'] ?>">
...
?>
							<td class="td-btn">
								<button type="button" class="btn-enter-anonym <?= $project['ENT_BTN_ROUND_YN'] == 1 ? 'round'  : '' ?>"
									style="font-size: 1.1rem; background: <?= $project['ENT_BTN_BG_COLR'] ?>; color: <?= $project['ENT_BTN_FONT_COLR'] ?>;"
									onclick="enter();"><?= $project['ENT_BTN_TEXT'] ?></button>
							</td>
							<?php
    } else {
?>
							...
							<td class="td-btn">
								<button type="button" class="btn-enter <?= $project['ENT_BTN_ROUND_YN'] == 1 ? 'round'  : '' ?>"
									style="font-size: 1.1rem; background: <?= $project['ENT_BTN_BG_COLR'] ?>; color: <?= $project['ENT_BTN_FONT_COLR'] ?>;"
									onclick="enter();"><?= $project['ENT_BTN_TEXT'] ?></button>
							</td>
							<?php
...
?>
	...
	<!-- 사전등록 Modal -->
	<div id="applyModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="applyModalTitle"
		aria-hidden="true">
...
?>
							<tr>
								...
								<td class="required">
									<input type="text" id="REQR_NM" name="REQR_NM" class="ent-info common-input w100" ent-info-title="성명"
										placeholder="" value="" />
								</td>
							</tr>
							<tr>
								...
								<td class="required">
									<input type="text" id="MBILNO" name="MBILNO" class="ent-info common-input w100" ent-info-title="연락처"
										placeholder="-는 제외하고 입력해주세요." value="" />
								</td>
							</tr>
							<!-- <tr>
...
?>
					...
					<div class="mt20">
						<p style="letter-spacing: -1px;">
							사전등록시 <a href="/agree/prvInfo" target="prv_info">개인정보 수집/이용</a> 및 <a href="/agree/marketing"
								target="marketing">마케팅 정보 제공</a>에 동의합니다.
						</p>
					</div>
				</div>
	...
	<!-- 관리자 로그인 Modal-->
	<div class="modal fade" id="admLoginModal" tabindex="-1" role="dialog" aria-labelledby="admLoginModalLabel"
		aria-hidden="true">
...
?>
		...
		$('input.ent-info').each(function() {
			// console.log($(this).attr('ent-info-title'));
			if ($(this).attr('ent-info-title') == '의사면허번호' || $(this).attr('ent-info-title') == '등록번호' || $(this).attr(
					'ent-info-title') == '면허번호' || $(this).attr('ent-info-title') == '생년월일') {
				// 필수일 경우만 체크
				if ($(this).hasClass('required')) {
	...
	function enter() {
		...
		$.ajax({
			type: 'POST',
			url: anonymUseYn == 0 ? '/stream/enter/<?= $project['PRJ_TITLE_URI'] ?>' :
				'/stream/enterAnonym/<?= $project['PRJ_TITLE_URI'] ?>',
			dataType: 'json',
			cache: false,
			data,
